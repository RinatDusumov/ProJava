...
class Graph {
    ...
    public Graph (List<Edge> edges) {
        ...
        for (Edge edge : edges) {
            vertex = Integer.max(vertex, Integer.max(edge.getSource(),
                    edge.getDestination()));
        }
        ...
        for (Edge current : edges) {
            ...
            /** выделение нового узла в списке смежности от источника к месту назначения */
            adjacencyList.get(current.getSource()).add(current.getDestination());

            /**
             * for undirected Graph
             * для ненаправленного графика
             *
             * allocate a new node in the adjacency list from destination to source
             * выделите новый узел в списке смежности от места назначения к источнику
             *
